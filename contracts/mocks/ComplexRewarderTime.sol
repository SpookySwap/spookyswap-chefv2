...
/// @author @0xKeno
contract ComplexRewarderTimeDisabled is IRewarder, Ownable {
    ...
    function onReward (uint256 pid, address _user, address to, uint256, uint256 lpToken) onlyMCV2 lock override external {
    ...
    function updatePool(uint256 pid) public returns (PoolInfo memory pool) {
        ...
        if (block.timestamp > pool.lastRewardTime) {
            ...
            if (lpSupply > 0) {
                uint256 time = block.timestamp.sub(pool.lastRewardTime);
                uint256 sushiReward = time.mul(rewardPerSecond).mul(pool.allocPoint) / totalAllocPoint;
                pool.accSushiPerShare = pool.accSushiPerShare + (uint128(sushiReward * ACC_TOKEN_PRECISION / lpSupply));
            }
