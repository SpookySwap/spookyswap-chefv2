...
describe.only("FetchHelper", function() {
  ...
  describe("Fetching Lp", async function() {
    it("Fetching LP data should succeed", async function() {
      ...
      for (let i = 0; i < Farms.length; i++) {
        ...
        consoleLog({
          [`${farm.lpSymbol}-v${farm.version}`]: deepKeyify(lpData)
        })
      }
  ...
  describe("Fetching Farms", async function() {
    it("Fetching Farm data should succeed", async function() {
      ...
      for (let i = 0; i < Farms.length; i++) {
        ...
        consoleLog({
          [`${farm.lpSymbol}-v${farm.version}`]: deepKeyify(farmData)
        })
      }
    ...
    it("Fetching LP data should succeed", async function() {
      ...
      for (let i = 0; i < Farms.length; i++) {
        ...
        if (farm.stakedUser) {
          ...
          consoleLog({
            [`${farm.lpSymbol}-staked-v${farm.version}`]: deepKeyify(stakedUserLpData),
          })
        }
        const unstakedUserLpData = await this.FetchHelper.fetchUserLpData(farm.unstakedUser, farm.lp, farm.version)
        consoleLog({
          [`${farm.lpSymbol}-unstaked-v${farm.version}`]: deepKeyify(unstakedUserLpData),
        })
      }
    ...
    it("Fetching Farm data should succeed", async function() {
      ...
      for (let i = 0; i < Farms.length; i++) {
        ...
        if (farm.stakedUser) {
          ...
          consoleLog({
            [`${farm.lpSymbol}-staked-v${farm.version}`]: deepKeyify(stakedUserFarmData),
          })
        }
        const unstakedUserFarmData = await this.FetchHelper.fetchUserFarmData(farm.unstakedUser, farm.pid, farm.version)
        consoleLog({
          [`${farm.lpSymbol}-unstaked-v${farm.version}`]: deepKeyify(unstakedUserFarmData),
        })
      }
  ...
  describe("Complex + Child Rewarder Farm", async function() {
    ...
    it("Fetching Farm User data should succeed", async function() {
      ...
      consoleLog({
        [`${this.complexRewardedFarm.lpSymbol}-staked-v${this.complexRewardedFarm.version}`]: deepKeyify(stakedUserFarmData),
      })
      ...
      consoleLog({
        [`${this.complexRewardedFarm.lpSymbol}-unstaked-v${this.complexRewardedFarm.version}`]: deepKeyify(unstakedUserFarmData),
      })
    })
  })
  ...
  // // UNCOMMENT TO TEST ALL FARMS
  // describe.only("Integration Test Farms JSON", async function() {
  //   it("Fetch spooky cdn farms data should succeed", async function() {

  //     const response = await axios.get(farmsUrl)

  //     const farmsConfig = response.data.farms

  //     // console.log("Fetch farms data")
  //     // await Promise.all(
  //     //   farmsConfig.map(({ pid, version, lpSymbol}, index) => {
  //     //     return this.FetchHelper.fetchFarmData(pid, version)
  //     //       .then(() => {
  //     //         process.stdout.write('.')
  //     //       })
  //     //       .catch((e) => {
  //     //         console.error(`Error fetching farm: ${lpSymbol} pid${pid} v${version}`, e)
  //     //       })
  //     //     }
  //     //   )
  //     // )

  //     console.log("Fetch user farms data")
  //     await Promise.all(
  //       farmsConfig.map(({ pid, version, lpSymbol}, index) => {
  //         return this.FetchHelper.fetchUserFarmData('0x3a7679E3662bC7c2EB2B1E71FA221dA430c6f64B', pid, version)
  //           .then(() => {
  //             process.stdout.write('.')
  //           })
  //           .catch((e) => {
